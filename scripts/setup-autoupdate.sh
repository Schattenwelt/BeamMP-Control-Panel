#!/usr/bin/env bash
###############################################################################
#  BeamMP Control Panel – Auto-Update einrichten
#
...
#      bash setup-autoupdate.sh
#  Optional andere Werte:
#      PANEL_REPO_URL=https://github.com/DEINNAME/BeamMP-Control-Panel.git \
#      UPDATE_TIME=04:30 bash setup-autoupdate.sh
#  Sofort einmal laufen lassen: mit  --run-now
###############################################################################
set -euo pipefail

REPO_URL="${PANEL_REPO_URL:-https://github.com/Schattenwelt/BeamMP-Control-Panel.git}"
BRANCH="${PANEL_REPO_BRANCH:-main}"
UPDATE_TIME="${UPDATE_TIME:-04:30}"
REPO_DIR="/opt/beammp-panel-src"
PANEL_DIR="/opt/beammp-panel"
BMP_USER="beammp"
BMP_HOME="/home/beammp"
BIN="/usr/local/bin/beammp-panel-update"
...
cat > "$BIN" <<EOS
#!/usr/bin/env bash
# Von beammp-panel-update.timer aufgerufen: Repo pullen + Panel deployen.
set -euo pipefail
REPO_DIR="$REPO_DIR"
PANEL_DIR="$PANEL_DIR"
BMP_USER="$BMP_USER"
...
cat > /etc/systemd/system/beammp-panel-update.service <<EOS
[Unit]
Description=BeamMP Panel Auto-Update (git pull + deploy)
...
cat > /etc/systemd/system/beammp-panel-update.timer <<EOS
[Unit]
Description=Taeglicher BeamMP-Panel-Auto-Update
...
systemctl enable --now beammp-panel-update.timer >/dev/null 2>&1 || systemctl enable beammp-panel-update.timer
ok "Timer aktiv."

if [ "${1:-}" = "--run-now" ]; then
    msg "Führe das Update jetzt einmal aus ..."
    systemctl start beammp-panel-update.service
    sleep 1
    journalctl -u beammp-panel-update.service -n 8 --no-pager -o cat | sed 's/^/    /'
fi

echo
ok "Auto-Update eingerichtet."
echo "    Nächste Läufe:   systemctl list-timers beammp-panel-update.timer"
echo "    Sofort updaten:  systemctl start beammp-panel-update.service"
echo "    Log ansehen:     journalctl -u beammp-panel-update.service"
echo "    Abschalten:      systemctl disable --now beammp-panel-update.timer"
